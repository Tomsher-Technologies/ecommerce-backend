...
export const registerSchema = zod.object({
    ...
    firstName: zod.string({ required_error: 'First name is required', }).min(2, 'First name is should be 2 chars minimum').refine(value => /^[A-Za-z\s]+$/.test(value), { message: 'Name should contain only alphabetic characters ' }),
...
export const guestRegisterSchema = zod.object({
    email: zod.string({ required_error: 'Email is required', }).email('Please provide a valid email address'),
    otpType: zod.enum(['phone', 'email'], {
        required_error: 'Otp type is required',
        invalid_type_error: 'Otp type must be either "phone" or "email"',
...
export const resetPasswordFormSchema = zod.object({
    ...
    otp: zod.string().min(4, { message: 'Otp must be at least 6 characters long' }).max(6).refine(value => value.trim() !== '', { message: 'Otp address cannot be empty' }),
