...
export const sliderSchema = zod.object({
    ...
    sliderImageUrl: zod.string().optional(),
    sliderImage: zod.any({ required_error: 'Slider image is required' }).nullable(),
...
export const sliderStatusSchema = zod.object({
    status: zod.string()
...
export const sliderPositionSchema = zod.object({
    position: zod.custom((val) => isNonNegativeInteger(val), {
        message: 'Position must be a non-negative integer or a string that can be converted to a non-negative integer'
    }),
});