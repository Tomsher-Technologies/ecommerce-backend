...
router.post('/', upload.any(), userPermissionMiddleware({ permissionBlock: 'banners', readOnly: 1 }), logResponseStatus, BannerController.create);
router.post('/:id', upload.any(), userPermissionMiddleware({ permissionBlock: 'banners', writeOnly: 1 }), logResponseStatus, BannerController.update);
router.post('/status-change/:id', userPermissionMiddleware({ permissionBlock: 'banners', writeOnly: 1 }), BannerController.statusChange);
router.post('/position-change/:id', userPermissionMiddleware({ permissionBlock: 'banners', writeOnly: 1 }), BannerController.positionChange);
